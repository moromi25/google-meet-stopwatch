...
window.addEventListener('load', () => {
    ...
    $(document).on('click', '.NHaLPe', recordMeetingTime);
...
function makeZeroFilledDate() {
    ...
    return '' + googleMeetStopWatchApp.endTime.getFullYear() + monthString + dateString;
}
...
function makeFormattedDate() {
    ...
    return '' + googleMeetStopWatchApp.endTime.getFullYear() + '年' + month + '月' + googleMeetStopWatchApp.endTime.getDate() + '日(' + dayOfWeekStr[dayOfWeek] + ')';
}
    ...
    chrome.storage.local.get(KEY).then((records) => {
        ...
        chrome.storage.local.set(newAllRecords, () => {
            chrome.storage.local.get(KEY, function (records) {
                console.log("【Google Meet Stopwatch Log】set meeting record successfully!");
...
function getMeetingTitle() {
    const meetingTitleBase = $('title').text();
    const meetingTitle = meetingTitleBase ? meetingTitleBase.replace('Meet - ', '') : location.pathname;
    return meetingTitle;
}
...
function outputConsoleLog(data) {
    ...
    if (data.elapsedTime === 0) {
        console.log('ミーティング【' + data.meetingTitle + '】の所要時間は 【1分未満】 でした。');
        return;
    }
    console.log('ミーティング【' + data.meetingTitle + '】の所要時間は 【約' + data.elapsedTime + '分】 でした。');
}